...
def remap(val, newMinVal, newMaxVal, oldMinVal, oldMaxVal):
...
class SliderHandler(object):
    ...
    def GetValue(self):
        value = self.FloatFormat % self.value
        if self.int_ctrl:
            return int(value)
        else:
            return float(value)
    ...
    def GetRangePositions(self):
        ...
        for startVal, stopVal, colour in self.ranges:
            startVal = remap(startVal, newMinVal, newMaxVal, oldMinVal, oldMaxVal)
            stopVal = remap(stopVal, newMinVal, newMaxVal, oldMinVal, oldMaxVal)
            if self.Horizontal:
                if self.Top:
                    rangePosition = [
                        startVal,
    ...
    def GetRangeMarkers(self):
        markers = []
        if len(self.ranges) == 1:
            startX, startY, width, height = self.GetRangePositions()[0][0]
            endX = startX + width
            endY = startY + height

            tickX1, tickY1, tickX2, tickY2 = self.GetTickPosition()[0]

            if self.Horizontal:
                startPoint1 = (startX, tickY1 - 1)
                startPoint2 = (startX, tickY2 + 1)
                stopPoint1 = (endX, tickY1 - 1)
                stopPoint2 = (endX, tickY2 + 1)

                if self.Top:
                    startPoint3 = (startX - 4, tickY2 + 1)
                    stopPoint3 = (endX + 3, tickY2 + 1)
                else:
                    startPoint3 = (startX - 4, tickY2 + 1)
                    stopPoint3 = (endX + 3, tickY2 + 1)
            else:
                startPoint1 = (tickX1, startY)
                startPoint2 = (tickX2, startY)
                stopPoint1 = (tickX1, endY)
                stopPoint2 = (tickX2, endY)

                if self.Left:
                    startPoint3 = (tickX2, endY - 3)
                    stopPoint3 = (tickX2, endY + 3)
                else:
                    startPoint3 = (tickX1, endY - 3)
                    stopPoint3 = (tickX1 , endY + 3)

            markers = [[startPoint1, startPoint2, startPoint3], [stopPoint1, stopPoint2, stopPoint3]]
...
class AGWSlider(wx.Panel):
    ...
    def __init__(
        ...
        if increment is None:
            increment = 100.0 / (maxValue - minValue)
...
# ----------------- PAINT
    ...
    def _DrawSliderBar(self, dc):
        ...
        if self._handler.SelRange:
            ...
            markers = self._handler.GetRangeMarkers()
            if markers:
                pens = [wx.TRANSPARENT_PEN] * 2
                brushes = [wx.Brush(wx.Colour(0, 0, 0))] * 2
                dc.DrawPolygonList(markers, pens, brushes)
...
# ----------------- PUBLIC
    ...
    def IsMinMaxLabel(self):
        """
        Gets if the min/max labels are being displayed.
    ...
    def SetMinMaxLabel(self, flag=True):
        """
        Turns on or off the displaying of the min and max labels.
    ...
    def IsValueLabel(self):
        """
        Gets if the value label is currently being displayed.
    ...
    def SetValueLabel(self, flag=True):
        """
        Turn on ior off the displaying of the value label.
    ...
    def IsInverse(self):
        """
        Returns whether ot not wx.SL_INVERSE was set.
    ...
    def GetValue(self):
        """
        Gets the current value of th slider.
    ...
    def SetValue(self, value):
        ...
        """
        if self._int_ctrl:
            self._handler.SetValue(int(value))
        else:
    ...
    def SetMin(self, minValue):
        ...
        """
        if self._int_ctrl:
            self._handler.SetMinValue(int(minValue))
        else:
    ...
    def SetMax(self, maxValue):
        ...
        """
        if self._int_ctrl:
            self._handler.SetMaxValue(int(maxValue))
        else:
    ...
    def SetRange(self, minValue, maxValue):
        """
        Sets the min and max values for the slider.
    ...
    def GetIncrement(self):
        """
        Gets the set increment for the slider.
        :rtype: int, float
        """
    ...
    def SetIncrement(self, increment):
        """
        Sets the increment for the slider.

        :type increment: int, float
        :rtype: None
        """
        if self._int_ctrl:
            self._handler.SetIncrement(int(increment))
        else:
    ...
    def AddSelection(self, min, max, colour):
        ...
        self._handler.SetSelection(min, max, colour)
...
if __name__ == '__main__':
    ...
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(RIGHT_THUMB_ACTIVE),pos=(20,10))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(RIGHT_THUMB),pos=(20,100))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(LEFT_THUMB_ACTIVE),pos=(20,180))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(LEFT_THUMB),pos=(20,260))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(TOP_THUMB_ACTIVE),pos=(20,340))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(TOP_THUMB),pos=(20,420))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(BOTTOM_THUMB_ACTIVE),pos=(20,500))
    bmp = wx.StaticBitmap(frame,-1,GetBitmap(BOTTOM_THUMB),pos=(20,580))
    ...
    for i, style in enumerate(STYLES):
        ...
        slider1 = AGWSlider(
            ...
            name=STYLE_NAMES[i] + ' (MULTIPLE RANGE INT)'
            # size=size
        )
        slider1.AddSelection(0.0, 70.0, (18, 171, 43))
        slider1.AddSelection(70.0, 90.0, (240, 229, 69))
        slider1.AddSelection(90.0, 100.0, (255, 0, 0))
        ...
        slider2 = AGWSlider(
            ...
            increment=0.1,
            style=style,
            name=STYLE_NAMES[i] + ' (NO RANGE FLOAT)'
            # size=size
        )
        ...
        sizer.Add(slider1, 0, wx.ALL, 10)
        sizer.Add(slider2, 0, wx.ALL, 10)
        sizer.AddStretchSpacer(0)
    ...
    slider2 = AGWSlider(
        ...
        name='SINGLE RANGE INT'
    )
    slider2.AddSelection(5, 40, (51, 153, 255))
    slider2.SetTickFreq(1)

    main_sizer.Add(slider, 0, wx.EXPAND | wx.ALL, 10)
    main_sizer.Add(slider2, 0, wx.EXPAND | wx.ALL, 10)
    frame.SetSizer(main_sizer)

    def on_top(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_TOP')
        evt.Skip()

    def on_bottom(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_BOTTOM')
        evt.Skip()

    def on_line_up(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_LINEUP' ':', obj.GetValue())
        evt.Skip()

    def on_line_down(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_LINEDOWN' ':', obj.GetValue())
        evt.Skip()


    def on_page_up(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_PAGEUP', ':', obj.GetValue())
        evt.Skip()


    def on_page_down(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_PAGEDOWN' ':', obj.GetValue())
        evt.Skip()


    def on_thumb_track(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_THUMBTRACK' ':', obj.GetValue())
        evt.Skip()


    def on_thumb_released(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_THUMBRELEASE' ':', obj.GetValue())
        evt.Skip()

    def on_scroll_changed(evt):
        obj = evt.GetEventObject()
        print(obj.GetName(), ':', 'EVT_SCROLL_CHANGED' ':', obj.GetValue())
        evt.Skip()

    frame.Bind(wx.EVT_SCROLL_TOP, on_top)
    frame.Bind(wx.EVT_SCROLL_BOTTOM, on_bottom)
    frame.Bind(wx.EVT_SCROLL_LINEUP, on_line_up)
    frame.Bind(wx.EVT_SCROLL_LINEDOWN, on_line_down)
    frame.Bind(wx.EVT_SCROLL_PAGEUP, on_page_up)
    frame.Bind(wx.EVT_SCROLL_PAGEDOWN, on_page_down)
    frame.Bind(wx.EVT_SCROLL_THUMBTRACK, on_thumb_track)
    frame.Bind(wx.EVT_SCROLL_THUMBRELEASE, on_thumb_released)
    frame.Bind(wx.EVT_SCROLL_CHANGED, on_scroll_changed)
    """
