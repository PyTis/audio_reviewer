...
``DD_DIR_MUST_EXIST``       0x200 The dialog will allow the user to choose only
  an existing folder. When this style is not given, a "Create new directory"
  button is added to the dialog (on Windows) or some other way is provided to
...
""" 
The dialog will allow the user to choose only an existing folder. When this
style is not given, a "Create new directory" button is added to the dialog (on
Windows) or some other way is provided to the user to type the name of a new
folder. 
"""
...
#    __file__ = __file__
    ...
    def __init__(self, parent, message=_("Choose one or more folders:"), title=_("Browse For Folders"),
        ...
        :param `agwStyle`: the AGW-specific dialog style; this can be a combination of the
         ...
         ===================== =========== ==================================================
         ``DD_NEW_DIR_BUTTON``       0x000 Enable/disable the "Make new folder" button
         ``DD_DIR_MUST_EXIST``       0x200 The dialog will allow the user to
                                           choose only an existing folder. When
                                           this style is not given, a "Create
                                           new directory" button is added to
                                           the dialog (on Windows) or some
                                           other way is provided to the user to
                                           type the name of a new folder.
...
#        staticText.SetFont(f)
        ...
        checkbox = wx.CheckBox(self, wx.NewId(), "Recursive")
        f = checkbox.GetFont()
        f.SetWeight(wx.BOLD)
        checkbox.SetFont(f)
        checkbox.SetToolTipString("Import from the selected folder, and all " \
          "of it's children folders.")

        self.checkbox = checkbox

        # Add the main wx.GenericDirCtrl        
#        mainSizer.Add(staticText, 0, wx.EXPAND|wx.ALL, 10)
        mainSizer.Add(self.checkbox, 0, wx.EXPAND|wx.ALL, 10)
